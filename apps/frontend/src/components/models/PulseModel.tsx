...
import {  useFrame, type EventHandlers, type InstanceProps, type MathProps, type ReactProps } from '@react-three/fiber';
import { OrbitControls, Environment, PerspectiveCamera, useGLTF, Merged, useAnimations } from '@react-three/drei';
...
export function Instances({ children, ...props }: { children?: React.ReactNode; [key: string]: any }) {
  const { nodes } = useGLTF('/models/dodekaden_orb.glb') as unknown as GLTFResult;
  
  const instances = useMemo(() => {
    // Apply crystal materials to all meshes
    const applyMaterials = (mesh: THREE.Mesh<THREE.BufferGeometry<THREE.NormalBufferAttributes, THREE.BufferGeometryEventMap>, THREE.Material | THREE.Material[], THREE.Object3DEventMap>, material: THREE.MeshPhysicalMaterial) => {
    ...
    return {
      ...
      TheCorners10: applyMaterials(nodes.TheCorners17__0, crystalCornerMaterial),
      TheCorners11: applyMaterials(nodes.TheCorners13__0, crystalCornerMaterial),
      TheCorners12: applyMaterials(nodes.TheCorners18__0, crystalCornerMaterial),
      TheCorners13: applyMaterials(nodes.TheCorners14__0, crystalCornerMaterial),
      TheCorners14: applyMaterials(nodes.TheCorners19__0, crystalCornerMaterial),
      TheCorners15: applyMaterials(nodes.TheCorners20__0, crystalCornerMaterial),
      TheCorners16: applyMaterials(nodes.TheCorners12__0, crystalCornerMaterial),
      TheCorners17: applyMaterials(nodes.TheCorners11__0, crystalCornerMaterial),
...
export function Model(props: JSX.IntrinsicAttributes & Mutable<Overwrite<Partial<Overwrite<THREE.Group<THREE.Object3DEventMap>, MathProps<THREE.Group<THREE.Object3DEventMap>> & ReactProps<THREE.Group<THREE.Object3DEventMap>> & Partial<EventHandlers>>>, Omit<InstanceProps<THREE.Group<THREE.Object3DEventMap>, typeof THREE.Group>, "object">>>) {
  const instances = useContext(context);
  const group = useRef<THREE.Group>(null);
  const { nodes, animations } = useGLTF('/models/dodekaden_orb.glb') as unknown as GLTFResult;
